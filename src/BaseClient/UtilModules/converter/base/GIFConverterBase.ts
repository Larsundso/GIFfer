...
export default abstract class GIFConverterBase {
 ...
 protected async convertWithAdaptiveResolution(mp4Path: string): Promise<string> {
  ...
  try {
   ...
   while (maxWidth >= 240) {
    ...
    console.log(`Generated GIF: ${maxWidth}px width, ${quality}% quality, ${fps}fps: ${fileSizeInMB.toFixed(2)}MB`);
    ...
    if (stats.size <= this.CLOSE_THRESHOLD) {
     ...
     if (quality > 70) {
      ...
      console.log(`Adjusted GIF: ${maxWidth}px width, ${newQuality}% quality: ${newSizeInMB.toFixed(2)}MB`);
